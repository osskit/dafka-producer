...
export const kafka = async (network: StartedNetwork, topics: string[], numPartitions: number = 1) => {
    ...
    await client.admin().createTopics({topics: topics.map((topic) => ({topic, numPartitions}))});
